...
describe('Entries API Tests', () => {
  ...
  test('"getEntryByAlias" should return an entry', () => {
    const alias = 'greg';
    return client.getEntryByAlias(alias).then(entry => {
      expect(entry).toBeTruthy();
      expect(entry._meta.entryAlias).toBe(alias);
    });
  });

  test('using the locale option should return a localized entry when available', () => {
    const alias = 'erik';
    return Promise.all([
  ...
  test('"getEntries" should return at least one entry', () => {
    ...
    return client.getEntries({ offset, size }).then(({ entries }) => {
  ...
  test('"getEntries" with the id param should return multiple entries', () => {
    ...
    return client.getEntries(params).then(({ entries }) => {
      expect(entries.length).toBe(params.id.length);
    });
  });

  test('"getEntries" with the typeId param should return multiple entries', () => {
    const params = Object.assign({}, paginationOptions, {
  ...
  test('"getEntries" with the typeId param for multiple typeIds should perform a request too', () => {
    const paramsArray = Object.assign({}, paginationOptions, {
      typeId: ['56ea7bca935f75032a2fd42c', '56ea7bca935f75032a2fd42c'],
    });
    return client.getEntries(paramsArray).then(({ entries }) => {
      expect(entries.length).toBeGreaterThan(0);
    });
  });

  test('"getEntries" with the alias param should return multiple entries, all with the given alias', () => {
    ...
    return client.getEntries(params).then(({ entries }) => {
      expect(entries.length).toBeGreaterThan(0);
      expect(entries.some(e => !aliases.includes(e._meta.entryAlias))).toBe(
        false
      );
    });
  });

  test('"getEntries" with the typeAlias param should return multiple entries, all with the given typeAlias', () => {
