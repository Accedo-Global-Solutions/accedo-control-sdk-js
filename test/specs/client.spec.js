...
describe('Appgrid Client creation', () => {
  ...
  test(
    'should not throw when sessionKey, appKey and deviceId are all passed',
    () => {
      const makeClient = () => factory({
        sessionKey: 'whatever',
        appKey: '56ea6a370db1bf032c9df5cb',
  ...
  test(
    'should throw when deviceId is passed without appKey and sessionKey',
    () => {
      const makeClient = () => factory({
        deviceId: 'stuff_here',
      });
  ...
  test(
    'should not throw but generate a deviceId when appKey is passed without deviceId',
    () => {
      let client;
      const makeClient = () => {
        client = factory({
