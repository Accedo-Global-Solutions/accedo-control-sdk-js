...
const isBrowser = require('../env').isBrowser();
...
const LOG_LEVELS_TO_SEND = [DEBUG, INFO, WARN, ERROR];
...
function doPostLog(level, log) {
  return post(`/application/log/${level}`, this.props.config, log);
}

function postLog(level, log) {
  return this.withSessionHandling(() => {
    const boundDoPostLog = doPostLog.bind(this, level, log);
    return !isBrowser
      ? boundDoPostLog()
      : this.getLogLevel().then(currentLevel => {
...
const stamp = stampit()
  ...
  .methods({
    ...
    sendLog(level, details, ...metadata) {
      if (!LOG_LEVELS_TO_SEND.includes(level)) {
